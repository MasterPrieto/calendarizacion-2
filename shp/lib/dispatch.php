<?php
/* AHORA requiere el config.php de SA.
 *
 */
...
function _log($message) {
  if (DISPATCH_DEBUG_ENABLED == true && php_sapi_name() !== 'cli') {
    $file = DISPATCH_DEBUG_LOG;
    $type = $file ? 3 : 0;
    error_log("{$message}\n", $type, $file);
  }
}
...
function site_path(){
  ...
  if (!$_path)
    $_path = rtrim(parse_url(CA_URL, PHP_URL_PATH),'/');
...
if (extension_loaded('mcrypt')) {
  ...
  function encrypt($decoded, $algo = MCRYPT_RIJNDAEL_256, $mode = MCRYPT_MODE_CBC) {
    ...
    if ( ($secret = DISPATCH_COOKIES_SECRET) == null)
...
//  if (($view_root = config('views.root')) == null)
  ...
  if ($layout !== false) {
    ...
    if ($layout == null) {
      $layout = DISPATCH_LAYOUT_DEFAULT;
...
function route($method, $pattern, $callback = null) {
  ...
  if (!in_array($method, array('GET', 'POST', 'DELETE', 'PUT')))
      error(500, 'Only GET, POST, DELETE and PUT are supported');
...
function flash($key, $msg = null, $now = false) {
  ...
  $f = (DISPATCH_COOKIES_FLASH ? DISPATCH_COOKIES_FLASH : '_F');
...
function dispatch($method = null, $path = null) {
  ...
  $path = ($path ? $path : $_SERVER['QUERY_STRING']);
